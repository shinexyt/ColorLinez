...
var gridSize = 40;
var ballRadius = 10;
var colors = ["red", "blue", "pink", "green", "brown"];
var nextColors = [];
var paths = [];
var eliminatedBalls = [];
var num = 0;
...
function onclick(event)
{
    ...
    else
    {
        ...
        if (paths.length > 0)
        {
            ...
            RemoveBall("white", map.startNode.x, map.startNode.y);
            move();
        }
        else
            map.resetArea();
...
{
    if (num > 0)
        RemoveBall("white", paths[num - 1].x, paths[num - 1].y);
    var x = paths[num].x;
    var y = paths[num].y;
...
function checkResult(isGetScore)
{
    ...
    if (eliminatedBalls.length >= 4)
    {
        ...
        RemoveBall("white", map.endNode.x, map.endNode.y);
        RemoveBalls();
    }
    else
    {
